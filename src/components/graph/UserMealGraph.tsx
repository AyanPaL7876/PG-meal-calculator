...
const UserMealGraph = ({ pgId }: { pgId: string }) => {
  ...
  const [mealData, setMealData] = useState<MealUser[]>([]);
  ...
  const fetchMealData = async () => {
    ...
    try {
      ...
      const mealSheet = pgData?.currMonth?.mealSheet || [];
      ...
      const userPromises = mealSheet.map(async (meal) => {
        ...
        if (!userSnap.exists()) return { id: meal.userId, name: "Unknown User" };
  ...
  const prepareUserTotalData = () => {
    return users.map((user, index) => {
      ...
      const totalMealCount = userMeal ? userMeal.details.reduce((total, detail) => 
        total + detail.sessions.length, 0) + (userMeal.extra || 0) : 0;
      
      // Count meal types
      let mealTypes = { breakfast: 0, lunch: 0, dinner: 0 };
      if (userMeal) {
  ...
  const prepareDailyData = () => {
    ...
    return users.map((user, index) => {
      ...
      if (userMeal) {
        const dayMeal = userMeal.details.find(detail => detail.date === selectedDate);
        if (dayMeal) {
    ...
    <Card className="bg-gray-900 text-white shadow-xl border-0">
      <CardHeader>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <CardTitle className="text-xl font-bold tracking-tight">Meal Analytics</CardTitle>
          ...
          <Select value={viewType} onValueChange={setViewType}>
            ...
            <SelectContent className="bg-gray-800 border-gray-700 text-white">
              <SelectItem value="total" className="focus:bg-gray-700">Total Meals</SelectItem>
              <SelectItem value="daily" className="focus:bg-gray-700">Daily View</SelectItem>
            </SelectContent>
          </Select>
        </div>
            ...
            <Select value={selectedDate} onValueChange={setSelectedDate}>
              ...
              <SelectContent className="bg-gray-800 border-gray-700 text-white">
                {availableDates.map(date => (
                  <SelectItem key={date} value={date} className="focus:bg-gray-700">
                    {formatDate(date)}
                  </SelectItem>
                ))}
              </SelectContent>
        ...
        {viewType === "total" ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Legend />
                    <Bar dataKey="breakfast" name="Breakfast" stackId="a" fill="#ffc658" />
                    <Bar dataKey="lunch" name="Lunch" stackId="a" fill="#82ca9d" />
                    <Bar dataKey="dinner" name="Dinner" stackId="a" fill="#8884d8" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-4">
              <h3 className="text-lg font-medium mb-4 text-center">Meal Type Distribution</h3>
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
          ...
          <div>
            <h3 className="text-lg font-medium mb-4">
              Meals for {selectedDate ? formatDate(selectedDate) : "Selected Date"}
            </h3>
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={dailyData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#2d3748" vertical={true} />
                    <XAxis dataKey="name" tick={{ fill: "#cbd5e0" }} />
                    <YAxis tick={{ fill: "#cbd5e0" }} domain={[0, 1]} tickCount={2} />
                    <Tooltip content={<CustomBarTooltip />} />
                    <Legend />
                    <Bar dataKey="breakfast" name="Breakfast" fill="#ffc658" maxBarSize={60} />
                    <Bar dataKey="lunch" name="Lunch" fill="#82ca9d" maxBarSize={60} />
                    <Bar dataKey="dinner" name="Dinner" fill="#8884d8" maxBarSize={60} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
              {MEAL_TYPES.map(type => {
                const count = dailyData.reduce((sum, user) => sum + user[type as keyof typeof user], 0);
                const percentage = dailyData.length ? Math.round((count / dailyData.length) * 100) : 0;
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-2.5">
                      <div 
                        className={`h-2.5 rounded-full ${
                          ...
                          type === 'lunch' ? 'bg-green-400' : 'bg-purple-400'
                        }`}
